    ...
    <div className="space-y-6">
        ...
        <>
          <section className="grid gap-4 md:grid-cols-3">
            <article className="glass rounded-2xl p-4">
              <p className="text-sm text-slate-500 dark:text-slate-400">Total Issued</p>
              <p className="text-3xl font-bold">{ownCredentials.length}</p>
            </article>
            <article className="glass rounded-2xl p-4">
              <p className="text-sm text-slate-500 dark:text-slate-400">Active</p>
              <p className="text-3xl font-bold text-emerald-600">
                {ownCredentials.filter((item) => item.status === 'active').length}
              </p>
            </article>
            <article className="glass rounded-2xl p-4">
              <p className="text-sm text-slate-500 dark:text-slate-400">Revoked</p>
              <p className="text-3xl font-bold text-rose-600">
                {ownCredentials.filter((item) => item.status === 'revoked').length}
              </p>
            </article>
          </section>
          ...
          <section className="glass rounded-2xl p-6">
            ...
            <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
              University role has permission to create student credentials.
            </p>
            <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
              <input
                className="rounded-xl border border-slate-300 bg-white px-4 py-3 dark:border-white/20 dark:bg-slate-950/60"
              ...
              <input
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, studentPassword: e.target.value }))}
                required
              />
              <input
                className="rounded-xl border border-slate-300 bg-white px-4 py-3 dark:border-white/20 dark:bg-slate-950/60"
            ...
            {newCredentialId && (
              <div className="mt-4 flex items-center gap-3 rounded-xl border border-blue-300 bg-blue-50 p-3 text-blue-700 dark:border-emerald-400/30 dark:bg-emerald-500/15 dark:text-emerald-200">
      ...
      <section>
        <h3 className="mb-3 text-xl font-semibold">
          {view === 'credentials' ? 'Issued Users Credentials' : 'Manage Issued Credentials'}
        </h3>
