    ...
    <div className="mx-auto flex min-h-screen w-full max-w-3xl items-center justify-center px-4">
      <form onSubmit={submit} className="glass w-full p-8">
        ...
        <div className="space-y-4">
          <input
            className="w-full border-4 border-black bg-white px-4 py-3 font-semibold dark:border-slate-200 dark:bg-slate-800"
          ...
          <button className="brutal-btn auth-btn w-full" disabled={isLoading}>
            {isLoading ? 'Creating...' : 'Create Merchant Account'}
          </button>
          <p className="text-center text-sm font-semibold">
            Already have merchant account?{' '}
            <Link to="/login?role=merchant" className="underline">
              Login
            </Link>
          </p>
        </div>
